...
# ── Publication-quality font & DPI defaults ──────────────────────────────────
plt.rcParams.update({
    'font.size':         16,
    'axes.titlesize':    18,
    'axes.labelsize':    16,
...
plot_dpi = st.sidebar.number_input("Figure DPI", min_value=72, max_value=1200, value=300, step=50)
...
if run_button:
    if not dicom_dir:
        ...
        try:
            ...
            axes[0, 0].set_ylabel('Signal Intensity', fontsize=16)
            axes[0, 0].set_title('Measured vs Predicted Signal', fontsize=18, fontweight='bold')
            ...
            axes[0, 1].set_ylabel('Residuals', fontsize=16)
            axes[0, 1].set_title('Residuals (Measured - Predicted)', fontsize=18, fontweight='bold')
            axes[0, 1].grid(True, alpha=0.3)
            axes[0, 1].tick_params(axis='both', labelsize=14)

            if model_type == 'T2-only':
                axes[0, 2].plot(T2_vals, f_recovered.flatten(), 'o-', linewidth=2)
                axes[0, 2].set_xlabel('T2 (s)', fontsize=16)
                axes[0, 2].set_xscale('log')
                axes[0, 2].set_title('Recovered T2 Distribution', fontsize=18, fontweight='bold')
                axes[0, 2].tick_params(axis='both', labelsize=14)
            elif model_type == 'T1-only':
                axes[0, 2].plot(T1_vals, f_recovered.flatten(), 'o-', linewidth=2)
                axes[0, 2].set_xlabel('T1 (s)', fontsize=16)
                axes[0, 2].set_xscale('log')
                axes[0, 2].set_title('Recovered T1 Distribution', fontsize=18, fontweight='bold')
                axes[0, 2].tick_params(axis='both', labelsize=14)
            else:
                ...
                axes[0, 2].set_xlabel('log10($T_{1}$)', fontsize=16)
                axes[0, 2].set_ylabel('log10($T_{2}$)', fontsize=16)
                axes[0, 2].set_title('Recovered log10 f($T_{1}$,$T_{2}$)', fontsize=18, fontweight='bold')
                axes[0, 2].tick_params(axis='both', labelsize=14)
                cbar = fig.colorbar(im, ax=axes[0, 2], fraction=0.046, pad=0.04)
                cbar.ax.tick_params(labelsize=13)

            axes[1, 0].hist(S, bins=20, alpha=0.7)
            axes[1, 0].set_xlabel('Signal Intensity', fontsize=16)
            axes[1, 0].set_title('Signal Distribution', fontsize=18, fontweight='bold')
            axes[1, 0].tick_params(axis='both', labelsize=14)

            axes[1, 1].hist(residuals, bins=20, alpha=0.7)
            axes[1, 1].set_xlabel('Residual Value', fontsize=16)
            axes[1, 1].set_title('Residual Distribution', fontsize=18, fontweight='bold')
            axes[1, 1].tick_params(axis='both', labelsize=14)

            (osm, osr), (slope, intercept, r) = stats.probplot(residuals, dist="norm", plot=axes[1, 2])
            axes[1, 2].set_title('Q-Q Plot of Residuals', fontsize=18, fontweight='bold')
